...
public static function getWLANSignal() {
	...
	$out->RSSI = exec('awk \'/0/ {print $4}\' /proc/net/wireless');
	$out->RSSIUnit = 'dBm';
	#$out->noise = exec('awk -F "[. ]" \'/0/ {print $12"dBm"}\' /proc/net/wireless');
	$out->noise = exec('awk -F "[. ]" \'/0/ {print $12}\' /proc/net/wireless');
	$out->noiseUnit = 'dBm';
	return $out;
}
...
public static function getTemperature() {
	$cmd = 'cat /sys/class/power_supply/bq27200-0/temp';
	# alternative:  'cat /sys/devices/platform/omap34xx_temp/temp1_input'
	return exec($cmd);
}

public static function getKeyboardSlideState() {
	$cmd = 'cat /sys/devices/platform/gpio-switch/slide/state';
	# alternative: 'cat /sys/devices/platform/camera_button/bus/devices/slide/state'
	return exec($cmd);
}
...
$output->sensors->temperature = N900::getTemperature();
...
$output->general->SERVER = arrayToObject($_SERVER);
...
function arrayToObject($array) {
	return json_decode(json_encode($array), FALSE); // FALSE = a stdclass object is returned
}
